...
reddit_path = "./bitcoin_comments.csv"
...
try:
    df_dean = pd.read_csv("./deanAddresscomment.csv")
except FileNotFoundError:
    df_dean = pd.DataFrame(columns=["Id_Key", "Address", "Testo", "Data", "Exchanger", "Tipo"])

try:
    df_unk = pd.read_csv("./unknownAddresscomment.csv")
except FileNotFoundError:
    df_unk = pd.DataFrame(columns=["Id_Key", "Address", "Testo", "Data"])
...
with open(reddit_path, newline="", encoding="ISO-8859-1") as filecsv:
    ...
    for i in lettore:
        ...
        if guessed_exchanger is None and type is None:
            list_row = [len(df_unk), i[1], i[2], i[3]]
            df_unk.loc[len(df_unk)] = list_row
        elif guessed_exchanger is not None and type is None:
            list_row = [len(df_dean), i[1], i[2], i[3], guessed_exchanger, np.nan]
            df_dean.loc[len(df_dean)] = list_row
        elif guessed_exchanger is not None and guessed_exchanger is not None:
            list_row = [len(df_dean), i[1], i[2], i[3], guessed_exchanger, type]
            df_dean.loc[len(df_dean)] = list_row
df_dean.to_csv('deanAddresscomment.csv', index=False)
df_unk.to_csv('unknownAddresscomment.csv', index=False)
